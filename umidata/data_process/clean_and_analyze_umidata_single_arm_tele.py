...
DEFAULT_INPUT_ROOT = REPO_ROOT / "umidata" / "single" / "20260526"
DEFAULT_OUTPUT_DIR = REPO_ROOT / "umidata" / "data_process" / "clean_report_20260526"
...
def compute_pair_deltas(
    ...
    if pair_len <= 0:
        return None, None
    left_ts = np.asarray(
        [parse_timestamp_from_name(name) for name in left_names[:pair_len]], dtype=np.float64
    )
    right_ts = np.asarray(
        [parse_timestamp_from_name(name) for name in right_names[:pair_len]], dtype=np.float64
    )
