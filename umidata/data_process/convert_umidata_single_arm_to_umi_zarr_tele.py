...
CAMERA_REL_PATH = Path(
    f"camera/color/pikaGripperFisheyeCamera"
    if FISHEYE
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Root directory for one date, containing task folders and episode* subfolders.",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def episode_sort_key(path: Path) -> Tuple[str, int, str]:
    ...
    if suffix.isdigit():
        return (str(path.parent), int(suffix), stem)
    return (str(path.parent), 10**9, stem)
...
def main() -> None:
    ...
    for episode_dir in tqdm(episode_dirs, desc="Converting episodes"):
        episode_name = str(episode_dir.relative_to(input_root))
        try:
            ...
            skipped.append((episode_name, str(exc)))
