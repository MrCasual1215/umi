...
class PolicyInference:
    ...
    def infer(self, payload: Dict, arm: Optional[str] = None) -> Tuple[Dict, Dict]:
        ...
        sent_action_xyz_quat_grip = self._apply_height_offset(action_xyz_quat_grip)
        if self.print:
        ...
        send_response = {
            "type": "action",
            "action_l": sent_action_xyz_quat_grip if selected_arm == "arm_l" else [],
            "action_r": sent_action_xyz_quat_grip if selected_arm == "arm_r" else [],
    ...
    def _build_env_obs(
        ...
        init_pose_raw = self._resolve_init_pose_raw(
            payload=payload,
            arm_name=arm_name,
            arm_payload=arm_payload,
        )
    ...
    @staticmethod
    def _resolve_init_pose_raw(payload: dict, arm_name: str, arm_payload: dict):
        init_pose_raw = arm_payload.get("init_pose")
        if init_pose_raw is not None:
            return init_pose_raw

        if "images" in payload and "poses" in payload and "grippers" in payload:
            return payload.get("init_pose")

        if isinstance(payload.get(arm_name), dict):
            return payload[arm_name].get("init_pose")
    ...
    def _apply_height_offset(self, actions: list) -> list:
